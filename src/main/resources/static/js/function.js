...
function reportErr(id) {
    openWin("/" + sitePath + "err.php?id=" + id, 400, 220, 350, 250, 0)
}
...
function submitComment(id) {
    ...
    ajax.postf(document.getElementById("f_comment"), function (obj) {
...
function getPageGoUrl(maxPage, pageDiv, type, listpagename) {
    ...
    switch (type) {
        ...
        case 3:
            ...
            if (goUrl.indexOf('//') == -1) {
                goUrl = regexpSplice(url, /(\/\/\S+_\d+?)(\.html|\.htm|\.shtml|\.shtm|\.asp|\.php)/, str);
            }
            break;
        case 5:
            str = (pageNum == 1) ? '' : "-" + pageNum;
...
function leaveWord() {
    ...
    ajax.postf(document.getElementById("f_leaveword"), function (obj) {
        if (obj.responseText == "ok") {
            viewLeaveWordList(1);
            alert('留言成功，多谢支持！');
...
function markscore1(vd, d, t, s, l, ac) {
    ...
    dc.onmouseout = function () {
        ...
        document.getElementById('MARK_B2').innerHTML = y;
        document.getElementById('MARK_B3').innerHTML = y > 0 ? alt[parseInt(y / l * (alt.length - 1))] : '请选择';
    }
    if (new Date().toGMTString() != new Date(document.lastModified).toGMTString()) return ajax.get(url[0], function (obj) {
        var a = obj.responseText
        try {
